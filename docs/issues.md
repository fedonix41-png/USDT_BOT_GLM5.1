...
## Устранённые проблемы (TelePay)

### ✅ Webapp: заглушки UserDashboard и AdminDashboard

**Проблема:** `UserDashboard.tsx` (51 строка) и `AdminDashboard.tsx` (20 строк) содержали минимальные заглушки без функциональности.

**Статус:** ✅ Устранено в TelePay integration.

- `UserDashboard.tsx` → ~1763 строк: 5 вкладок (Exchange, History, Profile, Referrals, Support), полная форма обмена, история заявок, KYC, sparkline chart
- `AdminDashboard.tsx` → ~1455 строк: 5 вкладок (Moderation, CRM, Support, Settings, Stats), double-tap confirm, CRM с поиском, редактор настроек
...
### P2: Support tickets — модуль не реализован в backend (TelePay)

**Проблема:** Frontend `UserDashboard` содержит вкладку Support с системой тикетов (`SupportTicket`, `AuthState.tickets`). Backend-эндпоинты и таблицы для тикетов отсутствуют.

**Файлы:** `webapp/src/types.ts` (типы `SupportTicket`), `webapp/src/components/user/UserDashboard.tsx`

**Решение:** Реализовать backend-модуль: таблица `support_tickets`, роутер, сервис, мапперы в `api/mappers.ts`.
...
| P2 | Support tickets — нет backend | `types.ts`, `UserDashboard.tsx` | Missing feature |
