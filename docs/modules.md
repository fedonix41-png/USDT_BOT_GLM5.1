...
1. Ввод начальной даты (формат ДД.ММ.ГГГГ)
2. Ввод конечной даты (валидация: end >= start)
3. Агрегация из `OrderService.get_statistics()`
4. Вывод: покупок, продаж, всего заявок
...
```
StatisticsStates:
  waiting_start_date → ожидание даты начала (ДД.ММ.ГГГГ)
  waiting_end_date   → ожидание даты окончания (ДД.ММ.ГГГГ)
```
