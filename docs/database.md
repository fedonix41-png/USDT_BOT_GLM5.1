...
- `payment_link_*` — зашифрованы через AES-GCM (префикс `gcm$`) или legacy AES-256-CBC (префикс `v1$`) через `EncryptionService`. Ключ шифрования хранится в `.env` (`ENCRYPTION_KEY`)
