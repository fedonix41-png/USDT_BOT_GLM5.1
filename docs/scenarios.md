...
> **Списание с баланса (WebApp):**
> - При создании заявки в WebApp, если внутренний баланс клиента **достаточен** (`user.balance >= amount_usdt`), сумма списывается автоматически с внутреннего баланса, а в заявке сохраняется префикс `[BALANCE_PAID]` для информирования оператора.
> - Если баланс **недостаточен** (`user.balance < amount_usdt`), заявка всё равно создается. Клиенту выводятся реквизиты кошелька обменника для внешнего перевода.
> - При отмене или отклонении заявки с префиксом `[BALANCE_PAID]` средства автоматически возвращаются клиенту.
...
### Снятие роли Оператора (Понижение до client)

**FSM:** `DemoteOperatorStates` (`app/fsm/role_states.py`)
...
Переходы:
  [👤⬇️ Снять оператора] → state = waiting_target_user
  Отправить: «Введите Telegram ID Оператора, с которого хотите снять права:»

  Пользователь вводит ID →
    Найти User в БД
    ↓ (не найден) → «Пользователь не найден. Он должен сначала запустить бота (/start).»
    Проверить: target_user.role == 'operator'
    ↓ (не оператор) → «Пользователь не является Оператором.»
  Установить role = client
  Отправить пользователю: «👤 С вас сняты полномочия Оператора бота обмена USDT.»
  Ответить админу: «✅ У пользователя @{username} снята роль Оператора.»
  + audit_log + уведомление в чаты через ARQ
```

### Снятие роли Админа (только SuperAdmin, Понижение до client)

**FSM:** `DemoteAdminStates` (`app/fsm/role_states.py`)

Аналогично, но:
- Кнопка `👑⬇️ Снять админа` доступна только SuperAdmin
- Проверить: target_user.role == 'admin'
- Устанавливается `role='client'`
- Отправить пользователю: «👑 С вас сняты полномочия Администратора бота обмена USDT.»
- Ответить админу: «✅ У пользователя @{username} снята роль Администратора.»
- Проверка: `min_role=RoleEnum.super_admin`
