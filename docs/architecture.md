...
| **OrderService** | Создание/отмена/завершение заявок, расчёт статистики, агрегация сумм |
| **RateService** | Получение текущего курса, установка нового курса (append в таблицу `rates`) |
| **SettingsService** | Управление флагами (`bot_enabled`, `buy_enabled`, `sell_enabled`) и реквизитами (с шифрованием) |
