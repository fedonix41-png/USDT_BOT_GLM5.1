...
Выполняются перед обработчиками, обеспечивают сквозную логику:

- **DBSessionMiddleware** — инъекция `AsyncSession` в `data["session"]` для каждого handler. Открывает сессию перед обработкой, коммитит после, откатывает при ошибке.
- **BotStatusMiddleware** — проверка флага `bot_enabled` (с кешем Redis на 30 сек). Если бот отключён и пользователь — клиент, перехватывает сообщение с ответом «Бот временно недоступен».
- **RoleGuardMiddleware** — проверка прав пользователя. Используется как фильтр на роутерах: `router.message.filter(RoleFilter(min_role=RoleEnum.admin))`.
